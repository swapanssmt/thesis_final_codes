...
% start modify
    if(~isvector(vmcmedium.layer))
        vmcmedium_out.nx = size(vmcmedium.layer,1);
        vmcmedium_out.ny = size(vmcmedium.layer,2);
        if(size(vmcmedium.layer, 3)>1)
           vmcmedium_out.nz = size(vmcmedium.layer,3);
           if(vmcmedium_out.nx*vmcmedium_out.ny*vmcmedium_out.nz ~= length(vmcmesh.H)/6)
               warning('Mesh size and medium size do not seem compatible.');
           end
        else
...
% end modify
    ...
    vmcmedium_out.layer = duplicateArray(vmcmedium.layer(:), size(vmcmesh.H,1));
