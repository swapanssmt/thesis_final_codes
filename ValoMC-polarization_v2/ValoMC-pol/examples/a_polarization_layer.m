...
h_disc=10.0/bins;
h_disc_1=5.0/bins;
x_arr = h_disc/2:h_disc:10.0-h_disc/2;
y_arr = h_disc/2:h_disc:10.0-h_disc/2;
z_arr = h_disc_1/2:h_disc_1:5.0-h_disc_1/2;
vmcmesh = createGridMesh(x_arr, y_arr, z_arr); % function provided by ValoMC
vmcmedium = createMedium(vmcmesh);
...
vmcmedium.scattering_coefficient = mus_2;
vmcmedium.absorption_coefficient = 0.01; % repeat six times
...
z_coord_4=z_coord(temp(:,4));

ck_1=find((z_coord_1 <= de) & (z_coord_2 <= de) & (z_coord_3 <= de) & (z_coord_4 <= de));
vmcmedium.scattering_coefficient(ck_1) = mus_1;
vmcmedium.layer(ck_1) = 1;


%% creating point source;
Point_LS=[(5.0+10^(-5)) (5.0+10^(-5)) 0.0];
x_coord=vmcmesh.r(:,1);
y_coord=vmcmesh.r(:,2);
z_coord=vmcmesh.r(:,3);
temp=vmcmesh.BH;
