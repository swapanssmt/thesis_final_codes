...
public:
  ...
  Array<double> r;               // Grid nodes and coordinates of centroid of unit sphere for angular discretization
...
void MC3D::CreatePhoton_f(Photon *phot)
{
  ...
  // Isotropic -- Photons initial direction probality density is uniform on a sphere
  // Wolfram Mathworld / Sphere Point Picking
  double r[3], costheta, sintheta, psi, sinpsi, cospsi;
  costheta = 1.0-2.0*UnifClosed();
  sintheta = sqrt(1.0 - costheta*costheta);
  psi = 2.0*M_PI*UnifClosed();
  cospsi = cos(psi);
  if (psi<M_PI)
  {
  ...
  {
    sinpsi = -1*sqrt(1.0-cospsi*cospsi);
  }
